...
3. 在实现上，这些函数都是泛型的拓展函数，
	1. lamda函数访问参数默认就是it形式 拓展函数可直接通过this访问到拓展对象
	2. lamda函数默认就是返回最行一行表达式的值；
## 协程
# Android
## Activity的启动流程
...
## Binder
1. Binder 是Android 特有的一种进程间通信方式，和其他通信方式相比，它基于mmap实现了"一次拷贝"
	1. 进程间共享内核空间，不共享用户空间，如果要传递数据需要从用户空间拷贝到内核空间，再从内核空间拷贝到用户空间；
	2. 利用mmap 将一块物理内存 同时映射到内核和接收端，并保存各自的虚拟地址偏移量，数据从发送端烤贝到内核之后，在根据地址偏移，接收端就可直接访问。
	3. 应用进程在启动时就会开启mmap映射一块1m-2page的内存 并在内核中创建一个binder_proc结构体保存当前进程的binder信息，
	4. binder_node 与 binder_ref的关系
...
## 插件化
1. class文件
	1. 组成apk的dex文件，经过ClassLoader加载后，会保存到数组中，后续在使用这些类的时会从这个数组中查找。通过ClassLoader将插件apk中dex 打包成一个数组 与 原来的数组进行合并，应用中就可以利用反射找到插件类。
	2. Android中存在内存快照保存类信息，快照是与Classloader相关联的，要解决这个问题 只能自定义Classloader，因为只有同一个Classloader加载的类才能访问到，需要全局替换
		1. 11
		2. 22
		3. 33
2. 资源文件
3. 四大组件
	1. 以Activity 为例，Activity必须要在Manifest注册
4. so文件
# jetpack
